...
function syncMcattNames() {
  ...
  ).map((input, index) => input.value.trim() || `เจ้าหน้าที่ ${index + 1}`);
...
function getStaffData(daysInMonth) {
  ...
  ).map((select) => Number.parseInt(select.value, 10) || 0);
...
function getScheduledDayCountForMonth(year, month) {
...
function getInitialUnitCursor(year, month, staffCount) {
  const cursor = Object.fromEntries(
    ROUTINE_UNITS.map((unitName) => [unitName, ROUTINE_START_INDEX[unitName] ?? 0])
  );
  ...
  if (
    compareYearMonth(year, month, CONTINUITY_BASE.year, CONTINUITY_BASE.month) <= 0
  ) {
    return cursor;
  }

  let rollingYear = CONTINUITY_BASE.year;
  let rollingMonth = CONTINUITY_BASE.month;

  while (compareYearMonth(rollingYear, rollingMonth, year, month) < 0) {
    const scheduledDays = getScheduledDayCountForMonth(rollingYear, rollingMonth);
    ROUTINE_UNITS.forEach((unitName) => {
      cursor[unitName] = (cursor[unitName] + scheduledDays) % staffCount;
    });
...
function buildReferenceSchedule(year, month, staffList, reference) {
  ...
  return {
    ...
    slotsFilled: schedule.reduce(
      (sum, row) => sum + Object.values(row.assignments).filter(Boolean).length,
      0
    ),
  };
}
...
function buildSchedule(year, month, staffList) {
  ...
  const unitCursor = getInitialUnitCursor(year, month, staffList.length);
...
function renderBalance(totals) {
  balanceWrap.innerHTML = totals
    ...
    .map((person) => {
      const unitTags = ROUTINE_UNITS.map((unitName) => `<span>${unitName}: ${person.units[unitName]}</span>`).join("");
