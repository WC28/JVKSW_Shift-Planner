...
function buildSchedule(year, month, staffList) {
  ...
  return {
    ...
    icodrDays: Array.from(icodrDays).sort((left, right) => left - right),
    referenceMode: false,
    referenceNote: null,
    daysScheduled: schedule.length,
    slotsRequired:
      schedule.length * ROUTINE_UNITS.length +
      schedule.filter((row) => row.icodr !== null || row.unfilled.includes("I-COD-R")).length,
    slotsFilled: schedule.reduce((sum, row) => {
      return sum + Object.values(row.assignments).filter(Boolean).length + (row.icodr ? 1 : 0);
    }, 0),
  };
...
function renderSpecialSection(summary) {
  ...
  const icodrText =
    summary.icodrDays.length > 0 ? summary.icodrDays.join(", ") : "เดือนนี้ไม่มีรอบ I-COD-R";
  ...
  specialWrap.innerHTML = `
    ...
    <article class="special-card">
      <h3>I-COD-R</h3>
      <p>วันศุกร์สัปดาห์ที่ 2 และ 4 ตรงกับวันที่ ${icodrText}</p>
    </article>
    <article class="special-card">
    ...
    .map((row) => {
      ...
      return `
        <tr>
          <td>${row.dayNumber}</td>
          <td>${row.weekday}</td>
          <td class="${row.icodr ? "" : (row.unfilled.includes("I-COD-R") ? "unfilled" : "")}">
            ${row.icodr ?? (row.unfilled.includes("I-COD-R") ? "ยังไม่สามารถจัดเวรได้" : "-")}
          </td>
          ${cells}
        </tr>
      `;
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    ...
    const items = [
      ...
      row.assignments["ER / OPD เด็ก"]
        ...
        : "",
      row.icodr
        ? `<div class="calendar-item icodr"><strong>I-COD-R</strong>: ${row.icodr}</div>`
        : "",
      holidayLabel
...
function exportCalendarPng() {
  ...
  ctx.fillText("OPD / ER-OPD เด็ก / I-COD-R / นิติจิตเวชคิวสำรอง", 32, 84);
  ...
  for (let slot = 0; slot < weeks * 7; slot += 1) {
    ...
    const lines = [
      ...
      row.icodr ? `I-COD-R: ${row.icodr}` : null,
    ].filter(Boolean);
    ...
    .map((person) => {
      ...
      return `
        <article class="balance-card">
          ...
          <p>
            I-COD-R ${person.special.icodr} | ตรวจงาน ${person.special.monthlyCheck} |
            Audit ${person.special.audit}
          </p>
...
function toCsv(schedule) {
  const headers = ["วันที่", "วัน", "I-COD-R", ...ROUTINE_UNITS];
